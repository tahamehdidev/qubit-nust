...
test("configured limits match the documented table", () => {
  ...
  assert.deepEqual(RATE_LIMITS.cohortJoinPerAccount, { windowMs: 15 * 60 * 1000, limit: 10 });
  assert.deepEqual(RATE_LIMITS.changePasswordPerAccount, { windowMs: 15 * 60 * 1000, limit: 5 });
});

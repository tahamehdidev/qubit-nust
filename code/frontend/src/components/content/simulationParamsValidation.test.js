...
test("amplitude_bar_chart: requires at least one amplitude", () => {
  expect(validateSimulationParams("amplitude_bar_chart", { amplitudes: [] })).toMatch(/at least one/);
});

test("amplitude_bar_chart: rejects a non-numeric amplitude", () => {
  expect(validateSimulationParams("amplitude_bar_chart", { amplitudes: [0.5, "oops"] })).toMatch(/number/);
});

test("amplitude_bar_chart: rejects mismatched labels/amplitudes length", () => {
  expect(
    validateSimulationParams("amplitude_bar_chart", { amplitudes: [0.5, 0.5], labels: ["only one"] })
  ).toMatch(/same number/);
});
...
test("topology_diagram: requires at least one qubit", () => {
  expect(validateSimulationParams("topology_diagram", { qubits: [], edges: [] })).toMatch(/at least one/);
});
...
test("topology_diagram: accepts a well-formed shape", () => {
  expect(
    validateSimulationParams("topology_diagram", {
      qubits: [{ id: 0, x: 0, y: 0 }, { id: 1, x: 1, y: 0 }],
      edges: [[0, 1]],
    })
  ).toBeNull();
});
...
test("basis_encoder: rejects a non-positive qubit count", () => {
  expect(validateSimulationParams("basis_encoder", { qubitCount: 0, defaultNumber: 0 })).toMatch(/positive/);
});

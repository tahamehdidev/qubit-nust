...
test("clicking the CNOT gate shows its description", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "CNOT gate, control q0, target q1, step 2" }));
...
test("renders a measurement gate on each of its qubits", () => {
  render(<GateCircuitDiagram params={threeQubitCircuitParams} />);
  expect(screen.getByRole("button", { name: "Measurement gate on q0, step 4" })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Measurement gate on q1, step 4" })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Measurement gate on q2, step 4" })).toBeInTheDocument();
});
